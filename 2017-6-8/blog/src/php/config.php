...
if ($_SERVER['SERVER_NAME'] === 'localhost') {
  $conn = new mysqli('localhost', 'root', 'root') or die(err(1, '数据库连接失败'));
} else {
  $conn = new mysqli('qdm216595431.my3w.com', 'qdm216595431', 'tianshihua') or die('mysql conn die');
}
...
if ($_REQUEST['action'] == 'chooseDB' && $_REQUEST['dbName']) {
  ...
  if ($_SERVER['SERVER_NAME'] === 'localhost') {
    ...
    $conn->select_db('qdm216595431_db') or die(err(1, '数据库选择失败3'));
  }
}
